...
def put_message_sqs(batch_id, sqs_persist):
    try:
        ...
        LoggerUtility.logError("Unable to put message to persist sqs for batch id - {} , sqs - {}".format(batch_id, sqs_persist))
        raise e
...
def generate_manifest_files(event, context):
    ...
    sqs_persist = os.environ['SQS_PERSIST_ARN'].rsplit(':', 1)[1]
